...
class YeelightDevice extends Device {
  constructor(adapter, id, light) {
    super(adapter, id);
    this.name = light.name || light.model;
    this.description = light.model;
    this.light = light;
    this.state = {
      power: light.state.on ? 'on' : 'off',
      bright: light.state.brightness,
      color_mode: light.state.color_mode,
      ct: light.state.ct,
      rgb: light.state.rgb,
      hue: light.state.hue,
      sat: light.state.sat,
    };
    this.support = light.methods;
    this['@type'] = ['Light'];
  ...
  onClientData(data) {
    try {
      ...
      this.state = Object.assign(this.state, message.params);
  ...
  colorTemp() {
    return parseInt(this.state.ct, 10);
  }

  color() {
    const mode = parseInt(this.state.color_mode, 10);

    switch (mode) {
      case 1:
        // RGB mode
        return `#${parseInt(this.state.rgb, 10).toString(16).padStart(6, '0')}`;
      case 3:
        // HSV mode
        return Color(
          {
            h: parseInt(this.state.hue, 10),
            s: parseInt(this.state.sat, 10),
            v: parseInt(this.state.bright, 10),
          }
        ).hex();
      default:
  ...
  brightness() {
    return parseInt(this.state.bright, 10);
  }
}
